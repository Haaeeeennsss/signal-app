...
export default function Landing() {
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0f] text-white">
      ...
      {/* Hero */}
      <section className="max-w-2xl mx-auto px-6 pt-12 pb-20 text-center">
        <div className="inline-flex items-center gap-2 bg-indigo-600/10 border border-indigo-500/20 rounded-full px-4 py-1.5 text-indigo-400 text-xs font-semibold uppercase tracking-wider mb-8">
        ...
        <button
          onClick={() => navigate('/quiz')}
          className="inline-flex items-center gap-2 bg-indigo-600 hover:bg-indigo-500 active:scale-[0.98] text-white font-black py-4 px-8 rounded-xl transition-all text-lg tracking-wide"
        >
          Take the quiz — 60 seconds ⚡
        </button>
        <p className="text-gray-700 text-xs mt-4">Free. No app download. No credit card.</p>
      </section>

      {/* Problem */}
      <section className="max-w-2xl mx-auto px-6 pb-20">
        <div className="bg-[#111118] border border-[#1f1f2e] rounded-2xl p-8">
      ...
      <section className="max-w-2xl mx-auto px-6 pb-20">
        ...
        <div className="space-y-4">
          {HOW_IT_WORKS.map(({ step, emoji, title, desc }) => (
            <div key={step} className="flex items-start gap-5 bg-[#111118] border border-[#1f1f2e] rounded-2xl p-5">
      ...
      <section className="max-w-2xl mx-auto px-6 pb-20">
        <div className="grid grid-cols-3 gap-4 text-center">
          {[
            ...
            <div key={label} className="bg-[#111118] border border-[#1f1f2e] rounded-2xl p-5">
              <div className="text-2xl font-black text-indigo-400 mb-1">{stat}</div>
      ...
      <section className="max-w-2xl mx-auto px-6 pb-20">
        ...
        <div className="space-y-3">
          {FAQS.map(({ q, a }) => (
            <div key={q} className="bg-[#111118] border border-[#1f1f2e] rounded-2xl p-5">
      ...
      <section className="max-w-2xl mx-auto px-6 pb-24 text-center">
        <div className="bg-gradient-to-br from-indigo-600/20 to-purple-600/10 border border-indigo-500/20 rounded-2xl p-10">
          ...
          <h2 className="text-3xl font-black mb-3">Ready to stop losing?</h2>
          ...
          <button
            onClick={() => navigate('/quiz')}
            className="bg-indigo-600 hover:bg-indigo-500 active:scale-[0.98] text-white font-black py-4 px-8 rounded-xl transition-all text-base tracking-wide"
          >
            Start the quiz →
          </button>
